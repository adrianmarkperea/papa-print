...
export default function QueueTable({ prints, isAdmin, onDelete, onMarkDone, onReorder, loading }) {
  ...
  function handleDragEnd(event) {
    ...
    if (over && active.id !== over.id) {
      const oldIndex = prints.findIndex(p => p.id === active.id)
      const newIndex = prints.findIndex(p => p.id === over.id)
      onReorder(arrayMove(prints, oldIndex, newIndex))
    }
  }
  ...
  if (prints.length === 0) {
    return (
      <div className="text-center py-16 border-2 border-dashed border-gray-200 rounded-xl">
        ...
        <p className="text-gray-500 font-medium">The queue is empty</p>
        <p className="text-gray-400 text-sm mt-1">Submit a print to get started!</p>
      </div>
    )
  }
  ...
  const thead = (
    <thead className="bg-gray-50 border-b border-gray-200">
      <tr>
        {isAdmin && <th className="pl-3 pr-1 py-3 w-8" />}
  ...
  return (
    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
      <SortableContext items={prints.map(p => p.id)} strategy={verticalListSortingStrategy}>
